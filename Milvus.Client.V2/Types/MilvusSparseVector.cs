...
public readonly struct MilvusSparseVector<T> : IEquatable<MilvusSparseVector<T>>
{
    ...
    public bool Equals(MilvusSparseVector<T> other)
    {
        ...
        // EqualityComparer<T>.Default is null-safe for reference types and applies the same NaN/bit semantics
        // as HashCode.Add, keeping Equals/GetHashCode consistent.
        for (int i = 0; i < values.Length; i++)
        {
            if (!EqualityComparer<T>.Default.Equals(values[i], otherValues[i]))
            {
                return false;
            }
        }
