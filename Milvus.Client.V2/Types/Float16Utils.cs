...
public static class Float16Utils
{
    ...
    public static ushort FloatToBf16(float value)
    {
        ...
        {
            // Inf / NaN: preserve the value instead of letting the rounding addition overflow/wrap
            // (a NaN bit pattern with low bits set would otherwise wrap and become +0.0 / -0.0).
            // Keep a NaN payload bit (mirroring the FP16 path) so a NaN does not read back as +Inf.
            ushort bf16 = (ushort)(bits >> 16);
            return (bits & 0x7FFFFF) != 0 && (bf16 & 0x7F) == 0 ? (ushort)(bf16 | 1) : bf16;
        }
    ...
    private static ushort FloatToHalfBits(float value)
    {
        ...
        // Round-to-nearest-even: round up when the round bit (bit 12) is set and either there is a
        // sticky remainder below it or the truncated result's LSB is odd (the tie breaks to even).
        uint roundBit = (mantissa >> 12) & 1;
        uint sticky = mantissa & 0xFFF;
        if (roundBit != 0 && (sticky != 0 || (result & 1) != 0))
        {
            result++;
        }
        return result;
