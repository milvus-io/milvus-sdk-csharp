...
public readonly struct MilvusIds : IEquatable<MilvusIds>
{
    ...
    /// <summary>
    /// Builds a <see cref="MilvusIds" /> from already-sliced per-query id arrays, or <c>default</c> when both
    /// are empty.
    /// </summary>
    internal static MilvusIds FromSlices(IReadOnlyList<long>? longIds, IReadOnlyList<string>? stringIds)
        => longIds is { Count: > 0 }
            ? new MilvusIds(longIds)
            : stringIds is { Count: > 0 }
                ? new MilvusIds(stringIds)
                : default;
