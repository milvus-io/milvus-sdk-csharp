    ...
    : IClassFixture<HybridSearchTests.HybridSearchCollectionFixture>, IDisposable
{
    ...
    public async Task HybridSearch_with_RRF_reranker()
    {
        ...
        Assert.True(results.Ids.LongIds.Count <= 3);
    }
    ...
    public async Task HybridSearch_with_output_fields()
    {
        ...
        Assert.Equal(CollectionName, results.CollectionName);
        Assert.NotNull(results.Ids.LongIds);
        Assert.True(results.FieldsData.Count >= 2);
        ...
        var results = await collection.HybridSearchAsync(
            [
                ...
                new SparseVectorAnnSearchRequest<float>(
                    "sparse_vector",
                    [new MilvusSparseVector<float>([0, 1], [1.0f, 1.0f])],
    ...
    [Fact]
    public void WeightedReranker_throws_for_null_weights()
    {
        Assert.Throws<ArgumentNullException>(() => new WeightedReranker(null!));
    }

    [Fact]
    public void VectorAnnSearchRequest_validation()
    {
        Assert.Throws<ArgumentNullException>(() => new VectorAnnSearchRequest<float>(
            null!,
            [new[] { 0.1f, 0.2f }],
            SimilarityMetricType.L2,
            limit: 3));

        Assert.Throws<ArgumentException>(() => new VectorAnnSearchRequest<float>(
            "",
            [new[] { 0.1f, 0.2f }],
            SimilarityMetricType.L2,
            limit: 3));
        ...
        Assert.Throws<ArgumentOutOfRangeException>(() => new VectorAnnSearchRequest<float>(
            "vector_field",
            [new[] { 0.1f, 0.2f }],
            SimilarityMetricType.L2,
            limit: 0));

        Assert.Throws<ArgumentNullException>(() => new VectorAnnSearchRequest<byte>(
            "vector_field",
            null!,
            SimilarityMetricType.Jaccard,
            limit: 3));

        Assert.Throws<ArgumentException>(() => new VectorAnnSearchRequest<byte>(
            "vector_field",
            [],
            SimilarityMetricType.Jaccard,
            limit: 3));
    ...
    [Fact]
    public void SparseVectorAnnSearchRequest_validation()
    {
        Assert.Throws<ArgumentNullException>(() => new SparseVectorAnnSearchRequest<float>(
            "vector_field",
            null!,
            SimilarityMetricType.Ip,
            limit: 3));

        Assert.Throws<ArgumentException>(() => new SparseVectorAnnSearchRequest<float>(
            "vector_field",
            [],
            SimilarityMetricType.Ip,
            limit: 3));
    ...
    [Fact]
    public async Task HybridSearch_throws_for_empty_requests()
    {
    ...
    [Fact]
    public async Task HybridSearch_throws_for_null_requests()
    {
        await Assert.ThrowsAsync<ArgumentNullException>(() => Collection.HybridSearchAsync(
            null!,
            new RrfReranker(),
            limit: 3));
    }
    ...
    [Fact]
    public async Task HybridSearch_throws_for_null_reranker()
    {
        await Assert.ThrowsAsync<ArgumentNullException>(() => Collection.HybridSearchAsync(
            [
                new VectorAnnSearchRequest<float>(
