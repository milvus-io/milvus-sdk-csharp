...
public enum SimilarityMetricType
{
    ...
    /// <summary>
    /// <see cref="Cosine" /> is used to measure the cosine of the angle between two non-zero vectors in a
    /// multidimensional space, reflecting the degree of similarity between them. The value ranges from -1 to 1, where 1
    /// indicates that the vectors are identical.
    /// </summary>
    /// <remarks>
    /// <para>
    ...
    /// </para>
    /// <para>
    /// It's widely used in applications involving natural language processing, search engines, and recommendation
    /// systems to calculate the similarity between documents or user preferences.
    /// </para>
    /// <para>
    /// This metric type is valid for float vectors only.
    /// </para>
    ...
    /// <summary>
    /// BM25 (Best Matching 25) is a ranking function used for full-text search. It ranks documents based on
    /// the query terms appearing in each document, regardless of their proximity within the document.
    /// </summary>
    /// <remarks>
    /// <para>
    /// This metric type is valid for sparse float vectors generated by the BM25 function.
    /// </para>
    /// <para>
    /// BM25 is commonly used for full-text search scenarios where you want to find documents that
