...
public partial class MilvusCollection
{
    ...
    /// <summary>
    /// Perform a full-text search using BM25 ranking function.
    /// </summary>
    /// <remarks>
    /// <para>
    /// This method is used for full-text search on collections that have a BM25 function defined.
    /// The search queries are raw text strings that will be tokenized and matched against the
    /// sparse vectors generated by the BM25 function.
    /// </para>
    /// <para>
    /// The collection must have a BM25 function that maps a VARCHAR field to a SPARSE_FLOAT_VECTOR field.
    /// </para>
    /// </remarks>
    ...
    /// </param>
    /// <param name="parameters">
    /// Various additional optional parameters to configure the search.
    /// </param>
    /// <param name="cancellationToken">
    /// The token to monitor for cancellation requests. The default value is <see cref="CancellationToken.None" />.
    /// </param>
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        Grpc.PlaceholderValue placeholderValue = new() { Tag = Constants.VectorTag };
        PopulateTextData(queries, placeholderValue);

        return await SearchInternalAsync(vectorFieldName, placeholderValue, SimilarityMetricType.Bm25, limit, parameters, cancellationToken)
            .ConfigureAwait(false);
    }
    ...
    private static Grpc.SearchRequest CreateSearchRequestFromAnnSearchRequest(AnnSearchRequest annRequest, string collectionName)
    {
        ...
        switch (annRequest)
        {
            ...
            case TextAnnSearchRequest textRequest:
                PopulateTextData(textRequest.Texts, placeholderValue);
                break;
            default:
    ...
    private static void PopulateTextData(IReadOnlyList<string> texts, Grpc.PlaceholderValue placeholderValue)
    {
