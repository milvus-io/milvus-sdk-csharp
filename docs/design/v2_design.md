...
e.g. `FieldData` (abstract, with `FieldData<T>` and concrete `*FieldData` leafs) and `IReranker`
(interface, with `RrfReranker`/`WeightedReranker`). Hybrid search's per-leg ANN queries are the sealed
`AnnSearchReq` DTO carried by `HybridSearchReq.SearchRequests`. This mirrors the Java/C++
...
- The server-side iterator is driven via the query/search request params (`batch_size`, `limit`,
  `offset`) and a `describe` call to resolve the primary key for cursor advance. The **query**
  iterator supports `offset` by seeking past it with a plain (non-iterator) query first, then
  advancing the pk cursor; only the **search** iterator rejects `offset`
  (`SearchIteratorReq.Validate` throws "Offset is not supported with a search iterator").
...
The database is sent on the wire in two complementary ways, matching the Java/C++ SDKs:

- **Client level** — the currently selected database is attached as the `dbname` gRPC metadata header
  (from `ConnectConfig.Database`, updated by `UseDatabaseAsync`). Every RPC carries it.
- **Request level** — most request DTOs expose an optional `DatabaseName` property. When set, the
  request's proto `db_name` field carries it and the server uses it instead of the header. When empty,
  the proto field is left blank and the header's database applies (Java `actualDbName` / C++ "use
  default database if empty" semantics).

Request-level `DatabaseName` therefore lets a single client operate on several databases concurrently
(create/drop/describe collections, insert/query/search, load, index, partition and alias operations),
while `UseDatabaseAsync` remains the way to change the default database. The schema and session-timestamp
caches key by the **resolved** database (`DatabaseName ?? current`), so cross-database operations never
collide in the caches.
...
internal static class RetryPolicy
{
    ...
    //   grpc.RpcException  -> retry unless StatusCode is in the 8-code blacklist (incl. Cancelled)
...
|---|---|---|---|
| gRPC blacklist (8 codes) | identical | identical | identical (`IGNORE_RETRY_CODES`) |
...
are retried, since PyMilvus (the reference client) and Java both retry them; only the 8 blacklisted
...
└── System/                      # Category=System (real container)
    ├── CollectionSystemTests.cs  IndexSystemTests.cs  AliasSystemTests.cs
    ├── DmlSystemTests.cs  DqlSystemTests.cs
    └── SystemTestBase.cs / MilvusV2Fixture.cs / MilvusTestContainer.cs
```
