...
public class UtilityReqTests
{
    ...
    [Fact]
    public void RunAnalyzer_field_mode_omits_analyzer_params()
    {
        // The server treats analyzer_params and (collection, field) as mutually exclusive; the field mode must
        // not serialize the default empty dictionary into "{}" (which the server would treat as an ad hoc analyzer).
        var request = new RunAnalyzerReq
        {
            Texts = new[] { "Hello Milvus!" },
            CollectionName = "coll",
        ...
        Assert.Equal("", grpc.AnalyzerParams);
        Assert.Equal("coll", grpc.CollectionName);
        Assert.Equal("text", grpc.FieldName);
    }

    [Fact]
    public void RunAnalyzer_rejects_analyzer_params_with_field_analyzer()
    {
        var request = new RunAnalyzerReq
        {
