...
public class RetryPolicyTests
{
    ...
    public async Task RateLimit_failures_are_retried()
    {
        ...
        Assert.Equal(5, server.Service.TotalCalls);   // Connect (1) + 3 failures + 1 success
    }
    ...
    public async Task Non_retryable_server_error_fails_immediately()
    {
        ...
        Assert.Equal(2, server.Service.TotalCalls);   // Connect (1) + 1 non-retried call
    }
    ...
    public async Task Gives_up_after_max_retries()
    {
        ...
        Assert.Equal(7, server.Service.TotalCalls);   // Connect (1) + 1 initial + 5 retries (MaxRetryTimes=5)
    }

    [Fact]
    public async Task Transport_unavailable_failures_are_retried()
    {
        using var server = new MockMilvusServer();
        using MilvusClientV2 client = new(Config(server));
        ...
        // The next 3 calls throw Unavailable (retryable transport code), then succeed.
        server.Service.RpcFailure = new RpcException(new Status(StatusCode.Unavailable, "unavailable"));
        server.Service.FailNextCalls = 3;

        await client.HasCollectionAsync(new HasCollectionReq { CollectionName = "coll" }, TestContext.Current.CancellationToken);

        Assert.Equal(5, server.Service.TotalCalls);   // Connect (1) + 3 transport failures + 1 success
    }

    [Fact]
    public async Task Non_retryable_transport_error_fails_immediately()
    {
        using var server = new MockMilvusServer();
        using MilvusClientV2 client = new(Config(server));
        ...
        // InvalidArgument is in the retry blacklist; the RpcException is wrapped into MilvusException
        // by InvokeAsync and is not retried.
        server.Service.RpcFailure = new RpcException(new Status(StatusCode.InvalidArgument, "bad arg"));
        server.Service.FailNextCalls = 1;
    ...
    [Fact]
    public async Task Transport_failure_preserves_original_grpc_status_code()
    {
        using var server = new MockMilvusServer();
        using MilvusClientV2 client = new(Config(server));
        ...
        // Unavailable is retried; after the retry budget is exhausted the final RpcException must surface with
        // its original gRPC status code preserved on the MilvusException.
        server.Service.RpcFailure = new RpcException(new Status(StatusCode.Unavailable, "unavailable"));
        server.Service.FailNextCalls = 100;

        MilvusException exception = await Assert.ThrowsAsync<MilvusException>(() =>
            client.HasCollectionAsync(new HasCollectionReq { CollectionName = "x" }, TestContext.Current.CancellationToken));

        Assert.Equal(StatusCode.Unavailable, exception.GrpcStatusCode);
