...
internal sealed class MilvusTestContainer : IAsyncDisposable
{
    ...
    public async ValueTask DisposeAsync()
    {
        ...
        {
            // Container removal is best-effort (stop_container in the script already swallows failures); a
            // failed stop at teardown must not mark an otherwise-green run as failed.
            Console.WriteLine($"Warning: failed to stop Milvus test container {_containerId}: {exception.Message}");
        }
    }
