...
def reap_label(grpc_port: int, health_port: int, minio_port: int) -> str:
    """Return the label identifying containers that occupy this run's ports."""
    return f"{REAP_LABEL}={grpc_port}-{health_port}-{minio_port}"


def reap_stale_containers(grpc_port: int, health_port: int, minio_port: int) -> None:
    """Remove non-running leftovers without disrupting another active test run."""
    result = docker(
        "ps",
        "--all",
        "--filter",
        f"label={reap_label(grpc_port, health_port, minio_port)}",
        "--format",
        "{{.ID}} {{.State}}",
        capture_output=True,
    )
...
def start_container(
    ...
    reap_stale_containers(grpc_port, health_port, minio_port)
    ensure_image(image)
    ...
    try:
        minio_result = docker(
            ...
            minio_name,
            "--label",
            reap_label(grpc_port, health_port, minio_port),
            "--env",
        ...
        result = docker(
            ...
            "--env",
            # Milvus reads its MinIO credentials from minio.accessKeyID/minio.secretAccessKey, whose
            # documented env vars are MINIO_ACCESS_KEY_ID/MINIO_SECRET_ACCESS_KEY (the MINIO_ROOT_* pair used
            # for the MinIO sidecar itself is inert on the Milvus side).
            f"MINIO_ACCESS_KEY_ID={MINIO_ACCESS_KEY}",
            "--env",
            f"MINIO_SECRET_ACCESS_KEY={MINIO_SECRET_KEY}",
            "--publish",
            f"{grpc_port}:19530",
            "--publish",
            f"{health_port}:9091",
